...
```
$ wtdbg2 -x rs -g 4.6m -t 4 -i ERR022075_PacBio.fastq -fo wtdbg2_raw
$ wtpoa-cns -t 4 -i wtdbg2_raw.ctg.lay.gz -fo wtdbg2_raw_assembly.fasta
```
...
-t 4
```
Modern Medaka auto-selects the model from the basecaller information in the reads, so you usually no longer need the `-m` flag. If you do need to set it, use the model matching your basecaller, e.g. `-m r1041_e82_400bps_sup_v5.2.0` for R10.4.1 `sup` data (the old `r941_*` models are for the retired R9.4.1 chemistry).
...
busco --list-datasets
```
Note: BUSCO datasets are now based on OrthoDB v12 (e.g. `gammaproteobacteria_odb12`); the `odb10` lineages below still work, but `--list-datasets` will show the current `odb12` options.
